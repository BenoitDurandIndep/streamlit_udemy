...
def add_data(task_doer, task_name, task_status, task_due_date):
    c.execute("INSERT INTO TASK(TASK_DOER,TASK_NAME,TASK_STATUS,TASK_DUE_DATE) VALUES (?,?,?,?)",
              (task_doer, task_name, task_status, task_due_date))
    con.commit()
...
def get_task_by_name(task_name: str, strict: bool = True):
    cond = f"TASK_NAME='{task_name}'"
    if not strict:
        cond = f"TASK_NAME LIKE'%{task_name}%'"

    c.execute(
        f"SELECT TASK_DOER,TASK_NAME,TASK_STATUS,TASK_DUE_DATE FROM TASK WHERE {cond}")
    data = c.fetchall()
    return data


def get_task_by_doer(task_doer: str, strict: bool = True):
    cond = f"TASK_DOER='{task_doer}'"
    if not strict:
        cond = f"TASK_DOER like '%{task_doer}%'"
